...
public:
	...
	void
	setfont(int h, int w, const char* fontface)
	{
	...
	void
	setfont(int h, int w, const wchar_t* fontface)
	{
		{
	...
	void
	gettext(int maxlen, char* text)
	{
		::SendMessageA(m_hwnd, WM_GETTEXT, ::WPARAM(maxlen), ::LPARAM(text));
	}

	void
	gettext(int maxlen, wchar_t* text)
	{
