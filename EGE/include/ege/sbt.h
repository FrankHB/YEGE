	...
	_maintain(TreeType t, bool flag)
	{
		...
		if(!flag)
		{
			if(left(t) && left(left(t))
					&& (!right(t) || size(left(left(t))) > size(right(t))))
			{
				t = _right_rotate(t);
			}
			else if(left(t) && right(left(t))
					&& (!right(t) || size(right(left(t))) > size(right(t))))
			{
		...
		{
			if(right(t) && right(right(t))
					&& (!left(t) || size(right(right(t))) > size(left(t))))
			{
				t = _left_rotate(t);
			}
			else if(right(t) && left(right(t))
					&& (!left(t) || size(left(right(t))) > size(left(t))))
			{
